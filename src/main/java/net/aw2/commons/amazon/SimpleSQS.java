 ...
 * 
 */
package net.aw2.commons.amazon;
...
public class SimpleSQS extends Object {
    ...
    private static final Logger log = Logger.getLogger(SimpleSQS.class);
    ...
    public List<Map> receiveMessage(String QueueUrl, int MaxNumberOfMessages, int VisibilityTimeout) throws Exception {
        ...
        while (resp == null && retries < MAX_RETRIES) {
            ...
            try {
                ...
                resp = getString(con.getInputStream(), "utf-8");
            }
            catch (Exception e) {
                retries = exponentialBackoff(retries, e);
            }
            finally {
        ...
        if (resp == null) {
            throw new Exception("Unable to get SQS response from URL: " + url);
        }
        ...
        for (int x = 0; x < messageList.size(); x++) {
            ...
            String bodyFromXML = getElements(messageList.get(x), "Body").get(0).toString();

//            StringBuilder body = new StringBuilder(8192);
//            body.append(bodyFromXML);
//            unescape(body, "&lt;", "<");
//            unescape(body, "&gt;", ">");
//            unescape(body, "&amp;", "&");
//            unescape(body, "&quot;", "\"");
...
//            mess.put("Body", body.toString());
            ...
            mess.put("Body", new String(Base64.decode(bodyFromXML), "utf-8"));
    ...
    public String sendMessage(String QueueUrl, String message) throws Exception {
        ...
        String messageBase64 = Base64.encodeBytes(message.getBytes());

        //System.err.println("sendMessage()  message=" + message + "\nbase64=" + messageBase64);
        //System.err.println("baseDecoded=" + new String(Base64.decode(messageBase64), "utf-8"));

        if (messageBase64.length() > 8192)
            throw new Exception("message is too large (<8KB)");

        TreeMap uriParams = createStandardParams("SendMessage");
        uriParams.put("MessageBody", messageBase64);
        uriParams.put("Signature", getSignature(false, QueueUrl, uriParams));

        int retries = 0;
        Map<String, String> results = null;

        while (results == null && retries < MAX_RETRIES) {
            try {
                results = IOUtil.postToConnection(new URL(QueueUrl), uriParams);
        ...
        if (results == null) {
            throw new Exception("Unable to send message to url: " + QueueUrl);
        }
        else {
    ...
    private String getString(InputStream is, String charEncoding) throws Exception {
        try {
    ...
    private int exponentialBackoff(int retries, Exception e) throws Exception {
        if (retries >= MAX_RETRIES) {
            ...
            // exponential backoff
            retries++;
            long sleepMillis = 250 * Math.round(Math.pow(2, retries));
            log.warn("Caught SQS Exception.  Retrying in " + sleepMillis + "ms");
            try { Thread.sleep(sleepMillis); } catch (Exception e2) { }
            return retries;
        }
    }
